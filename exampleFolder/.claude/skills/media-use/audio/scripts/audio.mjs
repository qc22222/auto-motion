...
import { normalizeIndexTts2Settings, runIndexTts2Batch } from "./lib/indextts2.mjs";
...
function projectFile(projectRoot, candidate, label) {
  const root = resolve(projectRoot);
  const target = resolve(root, String(candidate || ""));
  const rel = relative(root, target);
  if (!candidate || rel.startsWith("..") || isAbsolute(rel)) {
    throw new Error(`${label}必须是视频项目内的相对路径`);
  }
  return target;
}
...
const forceTts = has("force-tts");
const forceTtsLine = flag("force-tts-line", null);
...
const pronunciations = request.pronunciations && typeof request.pronunciations === "object"
  ? request.pronunciations
  : {};
const ttsCacheEnabled = !has("no-tts-cache") && request.tts?.cache !== false;
const ttsCacheRoot = join(hyperframesDir, ".harness", "audio-cache", "tts");
...
if (only.has("tts") && lines.length) {
  ...
  voiceId = await resolveVoiceId({
    ...
    userVoice: voiceOverride || (ttsProvider === "indextts2" ? request.referenceAudio : request.voice),
    lang,
  });
  console.error(`· tts: ${ttsProvider} · voice ${voiceId} · ${lines.length} line(s)`);
  if (ttsProvider === "indextts2") {
    if (request.referenceAudioAuthorized !== true) {
      die("IndexTTS2 referenceAudioAuthorized 必须明确为 true；只允许使用本人或已授权参考音频");
    }
    const referenceAudio = String(request.referenceAudio || "").trim();
    let referenceAbs;
    try {
      referenceAbs = projectFile(hyperframesDir, referenceAudio, "IndexTTS2 参考音频");
    } catch (error) {
      die(error.message);
    }
    if (!existsSync(referenceAbs)) die(`IndexTTS2 参考音频不存在：${referenceAudio}`);

    let indexSettings;
    try {
      indexSettings = normalizeIndexTts2Settings(settings);
    } catch (error) {
      die(error.message);
    }
    const referenceHashes = [sha256File(referenceAbs)];
    if (indexSettings.emotionMode === "audio") {
      let emotionAbs;
      try {
        emotionAbs = projectFile(hyperframesDir, indexSettings.emotionAudio, "IndexTTS2 情绪参考音频");
      } catch (error) {
        die(error.message);
      }
      if (!existsSync(emotionAbs)) die(`IndexTTS2 情绪参考音频不存在：${indexSettings.emotionAudio}`);
      referenceHashes.push(sha256File(emotionAbs));
    }
    const referenceAudioSha256 = sha256Text(referenceHashes.join(":"));
    ...
    for (const line of lines) {
      ...
      if (!/^[A-Za-z0-9._-]+$/u.test(id)) {
        anomalies.push(`line ${id || "<empty>"}: unsafe id — skipped`);
        continue;
      }
      ...
      const delivery = indexSettings.emotionMode === "delivery"
        ? [direction, line.delivery].map((item) => String(item || "").trim()).filter(Boolean).join("；")
        : "";
      ...
      const bypassCache = forceTts || forceTtsLine === id;
      if (ttsCacheEnabled && !bypassCache) {
        const cached = loadTtsCache({
        ...
        if (cached) {
          restoreTtsCache(cached, abs);
          const signal = inspectWavSignal(abs);
          if (signal.available && signal.silent) {
            anomalies.push(`line ${id}: cached voice is silent — omitted`);
          } else {
            resultById.set(id, {
              id,
              path: rel,
              duration_s: r3(cached.duration_s),
              words: withWordIds(cached.words),
              cache: cached.cache,
              qc: signal,
            });
            continue;
      ...
      pending.push({
        ...
        rel,
        abs,
        rawAbs: join(hyperframesDir, ".harness", "audio-temp", "indextts2", `${id}.wav`),
        cacheKey,
      });
    }
    ...
    if (pending.length > 0) {
      try {
        await runIndexTts2Batch({
          lines: pending.map((item) => item.line),
          hyperframesDir,
          referenceAudio,
          settings: indexSettings,
          direction,
          outputForLine: (line) => pending.find((item) => item.id === String(line.id)).rawAbs,
        });
      } catch (error) {
        die(error.message);
      }

      const synthesized = await mapWithConcurrency(pending, 1, async (item) => {
        mkdirSync(dirname(item.abs), { recursive: true });
        const ffmpegArgs = ["-y", "-loglevel", "error", "-i", item.rawAbs];
        if (Math.abs(speed - 1) > 0.000001) ffmpegArgs.push("-filter:a", `atempo=${speed}`);
        ffmpegArgs.push("-ar", "44100", "-ac", "1", "-c:a", "pcm_s16le", item.abs);
        const converted = await spawnP("ffmpeg", ffmpegArgs, { stdio: "ignore", windowsHide: true });
        if (converted.status !== 0 || !existsSync(item.abs)) {
          anomalies.push(`line ${item.id}: IndexTTS2 输出标准化失败 — omitted`);
          return null;
        }
        const wordArr = await transcribeWav({ wavRel: item.rel, lang, hyperframesDir });
        if (!Array.isArray(wordArr) || wordArr.length === 0) {
          anomalies.push(`line ${item.id}: IndexTTS2 字词对齐失败 — omitted`);
          return null;
        }
        const dur = ffprobeDuration(item.abs);
        if (!isFinite(dur) || dur <= 0) {
          anomalies.push(`line ${item.id}: bad voice duration — omitted`);
          return null;
        }
        const signal = inspectWavSignal(item.abs);
        if (signal.available && signal.silent) {
          anomalies.push(`line ${item.id}: synthesized voice is silent — omitted`);
          return null;
        }
        const cache = { enabled: ttsCacheEnabled, hit: false, key: item.cacheKey };
        if (ttsCacheEnabled) {
          try {
            saveTtsCache({
              cacheRoot: ttsCacheRoot,
            ...
            cache.enabled = false;
            anomalies.push(`line ${item.id}: TTS cache write failed (${cacheError.message})`);
          }
        }
        return {
          id: item.id,
          path: item.rel,
          duration_s: r3(dur),
          words: withWordIds(wordArr),
          cache,
          qc: signal,
        };
      });
      for (const result of synthesized.filter(Boolean)) resultById.set(result.id, result);
    }
    voices = lines.map((line) => resultById.get(String(line.id))).filter(Boolean);
  } else {
  const synthLine = async (line) => {
    ...
    let wordArr = words; // HeyGen/ElevenLabs：原生时间戳；Kokoro：二次对齐
    ...
    const signal = inspectWavSignal(abs);
    if (signal.available && signal.silent) {
      anomalies.push(`line ${id}: synthesized voice is silent — omitted`);
      return null;
    }
    const cache = { enabled: ttsCacheEnabled, hit: false, key: cacheKey };
    if (ttsCacheEnabled) {
      try {
        saveTtsCache({
          cacheRoot: ttsCacheRoot,
        ...
        cache.enabled = false;
        anomalies.push(`line ${id}: TTS cache write failed (${cacheError.message})`);
      }
    }
    return { id, path: rel, duration_s: r3(dur), words: withWordIds(wordArr), cache, qc: signal };
  };
  const results = await mapWithConcurrency(lines, ttsConcurrency, synthLine);
  voices = results.filter(Boolean);
  }
  for (const v of voices)
    console.error(`  voice ${v.id}: ${v.path} (${v.duration_s}s, ${v.words.length} words${v.cache?.hit ? ", cache hit" : ""})`);
}
...
const meta = {
  ...
  tts_cache: {
    enabled: ttsCacheEnabled,
    forced: forceTts,
    forced_line: forceTtsLine,
    hits: voices.filter((voice) => voice.cache?.hit).length,
    misses: voices.filter((voice) => voice.cache?.enabled && !voice.cache?.hit).length,
  },
  anomalies,
};
